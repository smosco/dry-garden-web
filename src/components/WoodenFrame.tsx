...
export default function WoodenFrame() {
  const frameWidth = 0.2  // 조금 더 두꺼운 틀
  const frameHeight = 0.12  // 약간 높은 높이
  const innerSize = GARDEN_SIZE
  const outerSize = innerSize + frameWidth * 2

  // 따뜻한 원목 색상 (자연스러운 나무 느낌)
  const woodColor = '#d4b896'
  const woodColorDark = '#b8936d'
    ...
    <group position={[0, frameHeight / 2, 0]}>
      ...
      {[
        ...
        <mesh key={i} position={[x, frameHeight * 0.1, z]} castShadow>
          <boxGeometry args={[frameWidth * 1.1, frameHeight * 1.3, frameWidth * 1.1]} />
          <meshStandardMaterial color={woodColorDark} roughness={0.75} metalness={0} />
        </mesh>
      ))}
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -frameHeight / 2 - 0.01, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#1a1816" roughness={1} metalness={0} />
      </mesh>
    </group>
  )
}
